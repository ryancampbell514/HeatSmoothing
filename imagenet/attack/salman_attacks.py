""" attacks suitable for stochastically-averaged models
    code taken from https://github.com/Hadisalman/smoothing-adversarial/blob/master/code/attacks.py """

""" THESE ATTACKS WERE MODIFIED FOR IMAGENET TOP5 """
...
def cohen_top5_classification(logits):
    Nreps, Nc = logits.shape
    counts = torch.zeros(Nc,device='cuda')

    for j in range(Nreps):
        y_pred = (logits[j]).argmax(dim=-1)
        counts[y_pred] += 1

    return counts.topk(k=5)[1]
...
class PGD_L2(Attacker):
    ...
    def _attack_mutlinoise(self, model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor,
        ...
        for i in range(self.steps):
            ...
            pred_top5 = cohen_top5_classification(logits)
            is_adv = labels not in pred_top5
            if is_adv:
                break
...
class DDN(Attacker):
    ...
    def _attack(self, model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor,
        ...
        for i in range(self.steps):
            ...
            is_adv = (labels in pred_top5) if targeted else (labels not in pred_top5)
            ...
            norm.mul_(1 - (2 * float(is_adv) - 1) * self.gamma)
